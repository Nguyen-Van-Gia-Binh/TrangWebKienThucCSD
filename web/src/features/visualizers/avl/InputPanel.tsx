...
import type { AVLOp } from "./generateSteps";
import { Play, Plus, Trash2, Search } from "lucide-react";

export function InputPanel({ defaultInput, onRun }: InputPanelProps<AVLOp[]>) {
  const [ops, setOps] = useState<AVLOp[]>(defaultInput);
  const [insertValue, setInsertValue] = useState("10");
  const [searchValue, setSearchValue] = useState("10");

  const addOp = (op: AVLOp) => setOps((prev) => [...prev, op]);
