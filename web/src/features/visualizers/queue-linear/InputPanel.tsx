...
function opLabel(op: QueueOp): string {
  return op.type === "enqueue" ? `enqueue(${op.value})` : `${op.type}()`;
}

export function InputPanel({ defaultInput, onRun }: InputPanelProps<QueueOp[]>) {
  const [ops, setOps] = useState<QueueOp[]>(defaultInput);
  const [value, setValue] = useState("10");

  const addOp = (op: QueueOp) =>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          onClick={() => {
            ...
            if (!Number.isNaN(v)) addOp({ type: "enqueue", value: v });
          }}
        >
          + enqueue
        </button>
        <button type="button" className={smallBtn} onClick={() => addOp({ type: "dequeue" })}>
          + dequeue
        </button>
      ...
      <div className="flex min-h-9 flex-wrap items-center gap-1.5 rounded-lg bg-neutral-50 p-2 dark:bg-neutral-950">
        {ops.length === 0 && (
          <span className="px-1 text-sm text-neutral-400 dark:text-neutral-600">
            Thêm thao tác enqueue / dequeue / peek rồi bấm Chạy…
          </span>
        )}
        {ops.map((op, i) => (
          <span
